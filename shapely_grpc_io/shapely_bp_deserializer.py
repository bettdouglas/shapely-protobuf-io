...
import shapely_grpc_io.geometry as gpb

class ShapelyBetterprotoDeserializer:
    def deserialize(self,gpbGeometry : gpb.Geometry) -> BaseGeometry:
        ...
        if enum_type == gpb.Type.POINT:
            ...
            # returns linestring
            return self.deserialize_line(gpbGeometry)
        raise NotImplementedError(gpbGeometry)
        

    def deserialize_point(self,gbpGeometry: gpb.Geometry) -> Point:
        return self.coordinate_to_point(gbpGeometry.coordinates[0])
    ...
    def deserialize_line(self, gpbGeometry : gpb.Geometry) -> LineString:
    ...
    def deserialize_polygon(self, gpbGeometry : gpb.Geometry) -> Polygon:
